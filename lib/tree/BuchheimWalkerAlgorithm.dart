...
class BuchheimWalkerAlgorithm extends Algorithm {
  ...
  Size run(Graph? graph, double shiftX, double shiftY) {
    ...
    dfsForNum(firstNode, 0);
    // firstWalk1(graph, firstNode, 0);
    dfs(graph, firstNode, 1);
    // graph.nodes
    //     .where((element) => element.nodeType == NodeType.main)
    //     .forEach((element) {
    //   final nodeData = getNodeData(element)!;
    //   dfs(graph, element);
    //   // int j = 1;
    //   // nodeData.successorNodes.forEach((e) {
    //   //   if (e.nodeType != NodeType.main) {
    //   //     e.position = Offset(
    //   //         element.position.dx + (150 * j) * (nodeData.number + 1),
    //   //         element.position.dy + (120));
    //   //     j *= -1;
    //   //   }
    //   // });
  ...
  void dfsForNum(Node node, int num) {
    var nodeData = getNodeData(node)!;
    nodeData.number = num;
    if (node.key!.value == 10) {
      nodeData.number = -3;
    }
    var i = 1;
    for (Node e in nodeData.successorNodes) {
      if (e.nodeType == NodeType.main) {
        dfsForNum(e, 0);
      } else {
        dfsForNum(e, i);
  ...
  // int dfsForMerge(Graph graph, Node node) {
  //   var nodeData = getNodeData(node)!;
  //   if(isLeaf(graph, node)){
  //     return 0;
  //   }
  //   // nodeData.number = num + nodeData.successorNodes.length;
  //   var i = 1;
  //   var t = 0;
  //   for (Node e in nodeData.successorNodes) {
  //     if (e.nodeType == NodeType.main) {
  //       dfsForNum(e, 0);
  //     } else {
  //       t += getNodeData(e)!.successorNodes.length;
  //       dfsForNum(e, i);
  //       i *= -1;
  //     }
  //   }
  ...
  void dfs(Graph graph, Node node, int l) {
    var nodeData = getNodeData(node)!;

    if (node.nodeType != NodeType.main) {
      if (nodeData.predecessorNodes.isNotEmpty &&
          getNodeData(nodeData.predecessorNodes.first)!.successorNodes.length ==
              1) {
        node.position = Offset(nodeData.predecessorNodes.first.x,
            nodeData.predecessorNodes.first.y + (240));
      } else {
        node.position = Offset(
            nodeData.predecessorNodes.first.x + (100) * (nodeData.number),
            nodeData.predecessorNodes.first.y + (120));
        var d = node.position;
      }
    }
    // print(nodeData.number);
    int k = -1;
    nodeData.successorNodes.forEach((e) {
      if (e.nodeType != NodeType.main) {
        k *= -1;
  ...
  void firstWalk1(Graph graph, Node node, int number) {
    var nodeData = getNodeData(node)!;
    nodeData.successorNodes.forEach((e) {
      if (e.nodeType != NodeType.main) {
        // getNodeData(e)!.number = number;
        // nodeData.number++;
        // print(e);
      }
      final len = successorsOf(node).length;
      firstWalk1(graph, e, number + 1);
    });
    // int j = 1;
    if (node.nodeType != NodeType.main) {
      nodeData.number = getNodeData(nodeData.predecessorNodes.first)!.number;
  ...
  void dfsPrint(Node node) {
    var nodeData = getNodeData(node)!;
    print('${node.key} ${nodeData.number}');
    var j = 1;
    // print(nodeData.number);
    nodeData.successorNodes.forEach((e) {
      dfsPrint(e);
  ...
  void secondWalk(Graph graph, Node node, double modifier) {
    ...
    var vertical = isVertical();
    var levelSeparation = configuration.levelSeparation;
    double? dy;
    if (node.nodeType == NodeType.alternate) {
      // levelSeparation = -40;
      if (nodeData.predecessorNodes.isNotEmpty &&
          getNodeData(nodeData.predecessorNodes.first)!.successorNodes.length !=
              1) {
        dy = nodeData.predecessorNodes.first.position.dy + 130;
        levelSeparation = -80;
        // depth++;
        // print(
        //     node.key.toString() + nodeData.predecessorNodes.length.toString());
      } else {
        // print(node.key.toString() +
        //     getNodeData(nodeData.predecessorNodes.first)!
        //         .successorNodes
        //         .toString());
        dy = nodeData.predecessorNodes.first.position.dy + 250;

        levelSeparation = -40;
      }
    }
    node.position = Offset(
        node.nodeType == NodeType.main && nodeData.predecessorNodes.isNotEmpty
            ? nodeData.predecessorNodes.first.x
            : (nodeData.prelim + modifier),
        dy ??
            (depth * (vertical ? minNodeHeight : minNodeWidth) +
                    depth * (levelSeparation))
                .ceilToDouble());

    if (node.nodeType == NodeType.alternate) {
      // print(node.key.toString() + nodeData.successorNodes.toString());
    }
  ...
  num getSpacing(Graph graph, Node? leftNode, Node rightNode) {
    ...
    //   separation = 12 ?? configuration.getSiblingSeparation();
    // }
  ...
  void initData(Graph? graph) {
    ...
    graph?.nodes
        .where((element) => element.nodeType == NodeType.main)
        .forEach((element) {
      element.position = Offset(100, i * 250);
      i++;
      print('object');
    });
  }
